...
void print_numbers_in_minefield(int x);
...
int main(void) {
    ...
    while (keep_looping == 1) { 
        ...
        //if the command is 4, then switch between the modes
        else if (initial == 4) { 
            gamemode_counter++;
    ...
    while (row < SIZE) {
        ...
        while (column < SIZE) {
            if (minefield[row][column] == VISIBLE_SAFE) {
                ...
                print_numbers_in_minefield(total_mines_to_print);
...
int check_validity(int x, int y) {
    ...
    int validity = (validity_a + validity_b) / 2;
    return validity;
}
...
int initial_two(int minefield[SIZE][SIZE], int x, int gamemode_counter){
    ...
    if (validity == 1) {
        ...
        printf("There are %d mine(s) in the square ", total_mines_overall);
...
//figure out which minefield to print
void print_minefield(int minefield[SIZE][SIZE], int x) {
    if (x == 1) {
        print_debug_minefield(minefield);
    } else if (x == 0) { 
...
//if statement to figure out what to print in the minefield depending 
//on the value of x
void print_numbers_in_minefield(int x) {
    if (x == 0) {
        printf("  ");
