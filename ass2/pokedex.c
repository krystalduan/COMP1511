...
void print_pokedex(Pokedex pokedex) {
    ...
    while (new != NULL && new_selected_dexnode != NULL) {
        if (new_selected_dexnode->found == 1) {
            if (new->pokemon == pokedex->selected_dexnode->pokemon) { 
                printf("--> ");
            } else { 
                printf("    ");
            }
            printf("#%03d: %s\n", pokemon_id(new->pokemon), pokemon_name(new->pokemon));
        } else if (new_selected_dexnode->found == 0) {
            if (new->pokemon == pokedex->selected_dexnode->pokemon) { 
                printf("--> ");
            } else { 
                printf("    ");
...
void prev_pokemon(Pokedex pokedex) {
    ...
    while (current_dexnode != NULL 
        && current_dexnode->next != pokedex->selected_dexnode) { 
...
void insert_pokemon_before_current(Pokedex pokedex, Pokemon pokemon) {
    ...
    while (current != NULL) { 
        ...
        current->next->pokemon = next_pokemon->pokemon;
...
int count_pokemon_of_type(Pokedex pokedex, char *type) {
    struct dexnode *new = pokedex->head;
    int counter = 0; 
    //counter = compare_strings(type_code_to_str(pokemon_first_type(new->pokemon), type);
    //int counter_two = 0;
    while (new != NULL) { 
       /* int result = strcmp(type_code_to_str(pokemon_first_type(new->pokemon)), type);
        printf("%d\n", result);
        printf("%s\n",type_code_to_str(pokemon_first_type(new->pokemon)));*/
        if (strcmp(type_code_to_str(pokemon_first_type(new->pokemon)), type) == 0) { 
            counter++;
            if (pokemon_second_type(new->pokemon) != type_str_to_code("None") 
            && strcmp(type_code_to_str(pokemon_second_type(new->pokemon)), type) == 0) {
                counter++;
            }
        }
      /*  char one[] = type_code_to_str(pokemon_first_type(new->pokemon));
        counter = compare_strings(&one, &type) + counter;
        counter_two = compare_strings(&type_code_to_str(pokemon_second_type(new->pokemon), &type) + counter_two;
...
int count_found_pokemon(Pokedex pokedex) {
    /*struct dexnode *new_selected = pokedex->selected_dexnode;
    
    int counter = 0; 
    while ( new_selected != NULL) { 
        if (new_selected->found == 1) { 
            counter++;
        }
