...
void pad_left(struct character_node *characters, char pad_character) {
    struct character_node *loop = characters; 
    int length = 0;
    while (loop != NULL) {
        length++; 
        loop = loop->next;
    ...
    int counter = 0;
    struct character_node *temp = characters;
   ...
   /* while (counter < length) { 
    ...
    /*struct character_node *new = malloc(sizeof(struct character_node)); 
    new->next = characters; 
    new->data = pad_character;*/
