...
int main(void) {
    ...
    while (keep_looping == 1) { 
        ...
        if (initial == 1 && counter_for_help < 3) {
            ...
            keep_looping = initial_three(minefield, keep_looping, no_of_mines, gamemode_counter);  
...
int initial_three (int minefield[SIZE][SIZE], int x, int no_of_mines, int gamemode_counter) { 
    ...
    if (validity == 1 && minefield[row_three][column_three] == 2) { 
        ...
        if (stop_game == 1) { 
            ...
            //printf("the value of keep looping inside the function befroe the definition is %d\n", x);
            x = 0; 
            //printf("the value of keep looping inside the function is %d\n", x);
        } else {
            print_minefield(minefield, gamemode_counter);
            /*if (gamemode_counter == 1) {
            print_debug_minefield(minefield);
